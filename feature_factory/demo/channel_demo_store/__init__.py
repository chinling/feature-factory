...
from feature_factory.demo.channel_demo_store.sales import Sales
...
class Store(Channel):
    ...
    def _create_data_source(self):
        try:
            ...
            store_sales_df2 = store_sales_df.join(store_df.withColumnRenamed("s_store_sk", "ss_store_sk"), ["ss_store_sk"])

            store_profit_by_div_df = store_sales_df2.groupBy(F.col("s_division_id")).agg(F.sum(F.col("ss_net_profit")).alias("net_profit_by_div"))
            joiner1 = Joiner(store_profit_by_div_df, on=["s_division_id"], how="left")
            joiner2 = Joiner(store_df.withColumnRenamed("s_store_sk", "ss_store_sk"), on=["ss_store_sk"], how="inner")

            self.add_source("store_sales", store_sales_df2, ["p_yyyymm"], joiners=[joiner1, joiner2])
